...
var cordovaAppPath = '../src/cordova/app/app/';
var cordovaAssetsPath = '../src/cordova/app/assets/';
...
// console.log("Installing cordova support files...");
// cp.execSync('npm install', {cwd: 'cordova'});

console.log("Configuring...");

// remove previous symlinks if they exist
try {
  if (fs.existsSync(resolve(cordovaAppPath))) {
    fs.unlinkSync(resolve(cordovaAppPath));
  }
  if (fs.existsSync(resolve(cordovaAssetsPath))) {
    fs.unlinkSync(resolve(cordovaAssetsPath));
  }
} catch (err) {
}
...
// if (!fs.existsSync(resolve(cordovaComponentsPath))) {
//     AttemptRootSymlink();
// }
...
if (!fs.existsSync(resolve(cordovaAppPath))) {
    ...
    console.log("  ", resolve(cordovaAppPath));
...
function AttemptRootSymlink() {
    ...
    if (process.platform === 'win32') {
      ...
      console.log("To automatically create a SymLink between your web app and cordova, we need root for a second.");
...
function createRootSymLink() {
    ...
    var p1 = resolve(AppPath + "/" + cordovaAppPath);
    ...
    p1 = resolve(AppPath + "/" + cordovaAssetsPath);
...
function createSymLink() {
    if (debugging) {
        console.log("Attempting to Symlink", webAppPath, cordovaAppPath);
    }
    fs.symlinkSync(resolve(webAppPath), resolve(cordovaAppPath), 'junction');
    fs.symlinkSync(resolve(webAssetsPath), resolve(cordovaAssetsPath), 'junction');
